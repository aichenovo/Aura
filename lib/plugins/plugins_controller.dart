...
import 'package:aura/plugins/plugin_validity_tracker.dart';
import 'package:aura/plugins/plugin_install_time_tracker.dart';
import 'package:aura/request/plugin.dart';
...
abstract class _PluginsController with Store {
  ...
  Future<void> loadAllPlugins() async {
    ...
    if (await newPluginDirectory!.exists()) {
      ...
      if (await pluginsFile.exists()) {
        ...
        final bool updated = await _ensureBundledPlugins([
          'assets/plugins/ziziys.json',
          'assets/plugins/qqqys.json',
          'assets/plugins/czzymovie.json',
          'assets/plugins/libvio.json',
          'assets/plugins/4kvm.json',
        ]);
        if (updated) {
          await savePlugins();
        }
  ...
  Future<bool> _ensureBundledPlugin(String assetPath) async {
    try {
      final jsonString = await rootBundle.loadString(assetPath);
      final plugin = Plugin.fromJson(jsonDecode(jsonString));
      final existingIndex = pluginList.indexWhere((p) => p.name == plugin.name);
      if (existingIndex == -1) {
        pluginList.add(plugin);
        return true;
      }
      final existing = pluginList[existingIndex];
      if (_compareVersion(plugin.version, existing.version) > 0) {
        pluginList[existingIndex] = plugin;
        return true;
      }
    } catch (_) {}
    return false;
  ...
  Future<bool> _ensureBundledPlugins(List<String> assetPaths) async {
    bool updated = false;
    for (final path in assetPaths) {
      final u = await _ensureBundledPlugin(path);
      updated = updated || u;
    }
    return updated;
  }
