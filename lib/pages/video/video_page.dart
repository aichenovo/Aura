...
import 'package:aura/pages/player/player_item.dart';
...
import 'package:aura/pages/player/episode_comments_sheet.dart';
import 'package:window_manager/window_manager.dart';
import 'package:aura/bean/widget/embedded_native_control_area.dart';
...
class _VideoPageState extends State<VideoPage>
  ...
  Widget get menuBar {
    ...
    final int safeCurrentRoad =
        (currentRoad >= 0 && currentRoad < roads.length) ? currentRoad : 0;
    final String currentRoadName = (roads.isNotEmpty &&
            roads[safeCurrentRoad].name.trim().isNotEmpty)
        ? roads[safeCurrentRoad].name
        : '播放列表${safeCurrentRoad + 1}';
    return Padding(
              ...
              (int i) => MenuItemButton(
                ...
                child: Container(
                  ...
                  child: Align(
                    ...
                    child: Text(
                      videoPageController.roadList[i].name.trim().isNotEmpty
  ...
  Widget get menuBody {
    ...
    final road = roads[safeCurrentRoad];
    if (road.data.isEmpty) {
      return Expanded(
        child: Center(
          child: Text(
            '暂无分集',
    ...
    for (var i = 0; i < road.data.length; i++) {
      final int count0 = i + 1;
      final urlItem = road.data[i];
      final String episodeLabel = (road.identifier.length >= count0 &&
              road.identifier[count0 - 1].trim().isNotEmpty)
          ...
          child: InkWell(
            onTap: () async {
              if (count0 == videoPageController.currentEpisode &&
                  videoPageController.currentRoad == safeCurrentRoad) {
                return;
              }
              KazumiLogger().i('VideoPageController: video URL is $urlItem');
              closeTabBodyAnimated();
              changeEpisode(count0, currentRoad: safeCurrentRoad);
            },
            child: Padding(
