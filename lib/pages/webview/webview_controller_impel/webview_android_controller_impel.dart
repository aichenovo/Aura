...
class WebviewAndroidItemControllerImpel
  ...
  Future<void> loadUrl(String url, bool useNativePlayer, bool useLegacyParser,
    ...
    if (url.contains('qqqys.com')) {
      await webviewController?.setSettings(
        settings: InAppWebViewSettings(
          userAgent:
              'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 sunshinetv',
        ),
      );
    }
    count = 0;
  ...
  Future<void> onLoadStart() async {
    // Handle qqqys fragment parameters for episode switching with enhanced debugging
    await webviewController?.evaluateJavascript(source: """
      (function() {
        if (window.location.hostname.includes('qqqys.com') && window.location.hash) {
          window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Fragment handler loaded: ' + window.location.href);
          window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Document ready state: ' + document.readyState);
          
          function handleQqqysFragment() {
            const hash = window.location.hash;
            window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Current hash: ' + hash);
            
            if (hash && hash.includes('sid=') && hash.includes('nid=')) {
              window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Processing fragment: ' + hash);
              
              const params = new URLSearchParams(hash.substring(1));
              const sid = params.get('sid');
              const nid = parseInt(params.get('nid'), 10);
              
              window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Parsed parameters - sid: ' + sid + ' (type: ' + typeof sid + '), nid: ' + nid + ' (type: ' + typeof nid + ')');
              
              // Wait for Alpine.js to initialize the playlist component
              let attempts = 0;
              const maxAttempts = 30;
              window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Starting to wait for playlist component...');
              ...
              const checkInterval = setInterval(() => {
                ...
                if (typeof window.playlist === 'function') {
                  ...
                  window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Playlist component found after ' + attempts + ' attempts');
                  
                  try {
                    const playlistInstance = window.playlist();
                    window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Playlist instance obtained');
                    ...
                    if (playlistInstance.selectSource) {
                      ...
                      window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✓ Source selected: ' + sid);
                    } else {
                      window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✗ selectSource method not found');
                    }
                    ...
                    if (playlistInstance.selectEpisode) {
                      setTimeout(() => {
                        ...
                        window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✓ Episode selected: ' + nid);
                        window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Final URL: ' + window.location.href);
                      }, 200);
                    } else {
                      window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✗ selectEpisode method not found');
                    }
                  } catch (e) {
                    window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✗ Error during selection: ' + e.message);
                    window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Error stack: ' + e.stack);
                  }
                } else if (attempts >= maxAttempts) {
                  clearInterval(checkInterval);
                  window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] ✗ Timeout: playlist component not found after ' + maxAttempts + ' attempts');
                  window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] window.playlist type: ' + typeof window.playlist);
                }
              }, 100);
            } else {
              window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Hash does not contain required parameters (sid and nid)');
            }
          }
          
          if (document.readyState === 'loading') {
            window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Waiting for DOMContentLoaded...');
            document.addEventListener('DOMContentLoaded', handleQqqysFragment);
          } else {
            window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Document already loaded, executing immediately');
            handleQqqysFragment();
          }
        } else {
          window.flutter_inappwebview.callHandler('LogBridge', '[qqqys] Conditions not met - hostname: ' + window.location.hostname + ', hash: ' + window.location.hash);
        }
      })();
    """);
