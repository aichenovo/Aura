...
import 'package:aura/modules/danmaku/danmaku_module.dart';
import 'package:mobx/mobx.dart';
import 'package:canvas_danmaku/canvas_danmaku.dart';
import 'package:aura/request/damaku.dart';
...
import 'package:aura/utils/syncplay.dart';
import 'package:aura/utils/external_player.dart';
...
abstract class _PlayerController with Store {
  ...
  Future<void> init(String url, {int offset = 0}) async {
    ...
    final String title = videoPageController.bangumiItem.nameCn.isNotEmpty
        ? videoPageController.bangumiItem.nameCn
        : videoPageController.bangumiItem.name;
    getDanDanmakuByTitle(title, episodeFromTitle);
  ...
  Future<void> getDanDanmakuByTitle(String title, int episode) async {
    if (danmakuLoading) {
      KazumiLogger()
    ...
    KazumiLogger().i('PlayerController: attempting to get danmaku [Title] $title');
    danmakuLoading = true;
    try {
      danDanmakus.clear();
      bangumiID = await DanmakuRequest.getBangumiIDByTitle(title);
      var res = await DanmakuRequest.getDanDanmaku(bangumiID, episode);
      addDanmakus(res);
    } catch (e) {
      KazumiLogger().w('PlayerController: failed to get danmaku [Title] $title',
          error: e);
    } finally {
      danmakuLoading = false;
    }
